interface MyNodeListInterface<T> {
    add(a: T);//添加元素
...
export class MyNode<T> {
    ...
    private _value: T;//值
    private _prev: MyNode<T>;//指向的上一个元素
    private _next: MyNode<T>;//指向的下一个元素
    constructor(value: T) {
        this._name = value + '';
        this._value = value;
        this._prev = null;
    ...
    set prev(item: MyNode<T>) {
        this._prev = item;
    }
    get prev(): MyNode<T> {
        return this._prev;
    }
    set next(item: MyNode<T>) {
        this._next = item;
    }
    get next(): MyNode<T> {
        return this._next;
...
export class MyNodeList<T> implements MyNodeListInterface<T> {
    private _count: number = 0;//记录元素个数
    private _header: MyNode<T>;//头元素
    private _tail: MyNode<T>;//尾元素
    constructor() {
        this._header = new MyNode<T>(null);
        this._header.name = 'header';
        this._tail = new MyNode<T>(null);
        this._tail.name = 'tail';
        this._header.prev = this._header.next = this._tail;
        this._tail.next = this._tail.prev = this._header;
    }
    add(a: T) {
        let item = new MyNode<T>(a);
